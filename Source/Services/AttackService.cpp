...
void StartAttack(Gameserver *Server, std::string Request, std::string Body)
{
    ...
    // TODO(Convery): Sync this with LAN, also maybe use a real hash.
    CurrentattackID = time(NULL);
    std::string Attackhash = va("%x", CurrentattackID);
    Attackhash.append(24 - Attackhash.size(), 'a');

    // Create the challenge.
    auto Response = MQEL_json::object();
                    Response["Result"]["AttackId"] = Attackhash;
