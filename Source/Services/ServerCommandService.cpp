...
void Handlecastlepurchase(BuyCommand *Command)
{
    Printfunction();
...
void Handleitempurchase(BuyCommand *Command)
{
    Printfunction();
}
...
void SendCommand(Gameserver *Server, std::string Request, std::string Body)
{
    ...
    for (auto &Item : Commands)
    {
        ...
        switch (Hash::FNV1a_32(Type.c_str()))
        {
            ...
            {
                Handlecastlepurchase(new BuyCommand(Item));
                break;
            }
            ...
            case Hash::FNV1a_32("HyperQuest.GameServer.Contracts.ClientIdleCommand, HyperQuest.GameServer.Contracts"):
            {
                /*
                    NOTE(Convery):
                    The client just tells us how many seconds
                    it has been idle, which can be used to notice
                    performance / network issues. Not too relevant.
                */
                break;
            }
            case Hash::FNV1a_32("HyperQuest.GameServer.Contracts.BuyHeroItemCommand, HyperQuest.GameServer.Contracts"):
            {
                Handleitempurchase(new BuyCommand(Item));
                break;
            }
            case Hash::FNV1a_32("HyperQuest.GameServer.Contracts.HeroEquipmentEquipCommand, HyperQuest.GameServer.Contracts"):
            {
                break;
            }
